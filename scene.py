...
class Physic_Scene:
    ...
    def add_static_object(self, name: str, object: ps.Physic_Static_Object) -> None:
    ...
    def new_object(self, name: str, object: bs.Transform_Object, dynamic: bool = True, weight: float = 1) -> ps.Physic_Object:
        ...
        if dynamic:
            ...
            if list(self.get_static_objects().keys()).count(name) <= 0:
                object = ps.Physic_Static_Object(self.get_base_struct(), object, weight = weight)
            ...
            for i in range(scene.get_scene_size()[0]):
                ...
                if list(parts.keys()).count(part) > 0: # If the part exists
                    if parts[part] != "":
                        ...
                        object = self.new_object(name, None, position = (1 * i, 3, 1 * j), scale = (1, 5, 1), texture_path = parts[part].get_texture_path(), type = type)
                else: # If the part does not exist
                    print("Matix scene : Warning !! The part \"" + part + "\" into the map \"" + scene.get_map_path() + " \" for loading into the scene \"" + self.get_name() + "\" does not exist.")
    
    def new_object(self, name: str, parent: bs.Transform_Object = None, position = (0, 0, 0), rotation = (0, 0, 0), scale = (1, 1, 1), texture_path: str = "", type: str = "cube") -> bs.Transform_Object:
        """Create a new object into the scene and return it
